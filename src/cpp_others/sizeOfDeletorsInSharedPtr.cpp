...
int main() {
    std::shared_ptr<FILE_S> uptr1(fopen_ss("test_file.txt"));
    std::cout << sizeof(uptr1) << std::endl;

    std::cout << "sizeof(uptr1) " << sizeof(uptr1) << std::endl;
    std::shared_ptr<FILE_S> uptr2(fopen_ss("test_file1.txt"),
                                                     FileCloserFunc);

    std::cout << "sizeof(uptr2) " << sizeof(uptr2) << std::endl;

    std::shared_ptr<FILE_S> uptr3(
